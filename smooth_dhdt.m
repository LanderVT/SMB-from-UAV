function matrix_smooth = smooth_dhdt(matrix, icethick, type)
    ...
    for i = 1:size(fluxer, 1)
        for j = 1:size(fluxer, 2)
            if isnan(fluxer(i, j))
                ...
                elseif type == "gaus"
                    a = Gaussian_filter(wind*2+1, (wind*2+1));
                    matrix_inter = nanconv(fluxer,a, 'nanout');
                    new_val = matrix_inter(i, j);
                end
